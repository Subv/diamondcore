...
bool OPvPCapturePoint::Update(uint32 diff)
{
    ...
    CellPair p(Diamond::ComputeCellPair(m_capturePoint->GetPositionX(), m_capturePoint->GetPositionY()));
    Cell cell(p);
    cell.data.Part.reserved = ALL_DISTRICT;
    cell.SetNoCreate();   

    {
    Diamond::AnyUnitInObjectRangeCheck u_check(m_capturePoint, radius);
    Diamond::UnitListSearcher<Diamond::AnyUnitInObjectRangeCheck> searcher( m_capturePoint, targets, u_check);

    TypeContainerVisitor<Diamond::UnitListSearcher<Diamond::AnyUnitInObjectRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
    TypeContainerVisitor<Diamond::UnitListSearcher<Diamond::AnyUnitInObjectRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);
