...
bool ChatHandler::HandleGoCreatureCommand(const char* args)
{
    ...
    else
    {
        ...
        else
        {
            ...
            QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM creature, creature_template WHERE creature_spawns.id = creature_template.entry AND creature_template.name "_LIKE_" "_CONCAT3_("'%%'","'%s'","'%%'"), name.c_str());
            if (!result)
            {
                SendSysMessage(LANG_COMMAND_GOCREATNOTFOUND);
                SetSentErrorMessage(true);
...
bool ChatHandler::HandleGoObjectCommand(const char* args)
{
    ...
    else
    {
        ...
        else
        {
            std::string name = pParam1;
            WorldDatabase.escape_string(name);
            QueryResult *result = WorldDatabase.PQuery("SELECT guid FROM gameobject, gameobject_template WHERE gameobject_spawns.id = gameobject_template.entry AND gameobject_template.name "_LIKE_" "_CONCAT3_("'%%'","'%s'","'%%'"), name.c_str());
            if (!result)
            {
                SendSysMessage(LANG_COMMAND_GOOBJNOTFOUND);
                SetSentErrorMessage(true);
    ...
    if (*args)
    {
        ...
        else
        {
            ...
            result = WorldDatabase.PQuery(
                ...
                "FROM gameobject,gameobject_template WHERE gameobject_template.entry = gameobject_spawns.id AND map = %i AND name "_LIKE_" "_CONCAT3_("'%%'","'%s'","'%%'")" ORDER BY order_ ASC LIMIT 1",
...
/**
 * Add a waypoint to a creature_spawns.
 *
