...
void OPvPCapturePointEP_NPT::ChangeState()
{
    if (fabs(m_value) == m_maxValue)  // state won't change, only phase when maxed out!
    {
        ...
        if ( m_OldState == OBJECTIVESTATE_ALLIANCE && m_OldState != m_State )
        {
            sWorld.SendZoneText(EP_GraveYardZone,sObjectMgr.GetStringForDBCLocale(LANG_OPVP_EP_LOOSE_NPT_A));
            ((OutdoorPvPEP*)m_PvP)->EP_Controls[EP_NPT] = 0;
        }
        else if ( m_OldState == OBJECTIVESTATE_HORDE && m_OldState != m_State )
        {
            sWorld.SendZoneText(EP_GraveYardZone,sObjectMgr.GetStringForDBCLocale(LANG_OPVP_EP_LOOSE_NPT_H));
            ((OutdoorPvPEP*)m_PvP)->EP_Controls[EP_NPT] = 0;
        }
...
void OPvPCapturePointEP_CGT::ChangeState()
{
    if (fabs(m_value) == m_maxValue)  // state won't change, only phase when maxed out!
    {
        ...
        if ( m_OldState == OBJECTIVESTATE_ALLIANCE && m_OldState != m_State )
        {
            sWorld.SendZoneText(EP_GraveYardZone,sObjectMgr.GetStringForDBCLocale(LANG_OPVP_EP_LOOSE_CGT_A));
            ((OutdoorPvPEP*)m_PvP)->EP_Controls[EP_CGT] = 0;
        }
        else if ( m_OldState == OBJECTIVESTATE_HORDE && m_OldState != m_State )
        {
            sWorld.SendZoneText(EP_GraveYardZone,sObjectMgr.GetStringForDBCLocale(LANG_OPVP_EP_LOOSE_CGT_H));
            ((OutdoorPvPEP*)m_PvP)->EP_Controls[EP_CGT] = 0;
        }
...
void OPvPCapturePointEP_PWT::ChangeState()
{
    if (fabs(m_value) == m_maxValue)  // state won't change, only phase when maxed out!
    {
        ...
        if ( m_OldState == OBJECTIVESTATE_ALLIANCE && m_OldState != m_State )
        {
            sWorld.SendZoneText(EP_GraveYardZone,sObjectMgr.GetStringForDBCLocale(LANG_OPVP_EP_LOOSE_PWT_A));
            ((OutdoorPvPEP*)m_PvP)->EP_Controls[EP_PWT] = 0;
        }
        else if ( m_OldState == OBJECTIVESTATE_HORDE && m_OldState != m_State )
        {
            sWorld.SendZoneText(EP_GraveYardZone,sObjectMgr.GetStringForDBCLocale(LANG_OPVP_EP_LOOSE_PWT_H));
            ((OutdoorPvPEP*)m_PvP)->EP_Controls[EP_PWT] = 0;
        }
