...
//            }
    ...
    else
    {
        if (Unit* caster = GetCaster())
        {
            if (triggerTarget->GetTypeId() != TYPEID_UNIT || !sScriptMgr.EffectDummyCreature(caster, GetId(), GetEffIndex(), (Creature*)triggerTarget))
...
void Aura::HandleAuraDummy(bool apply, bool Real)
{
    ...
    if (target->GetTypeId() == TYPEID_UNIT)
        sScriptMgr.EffectAuraDummy(this, apply);
}
            ...
            switch (spell->Id)
            {
                ...
                case 51690:
                {
                    ...
                    std::list<Unit*> targets;
                    {
                        ...
                        Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(target, target, radius);
                        Diamond::UnitListSearcher<Diamond::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(target, targets, u_check);
...
void Aura::HandleAuraControlVehicle(bool apply, bool Real)
{
    ...
    if (target->GetTypeId() != TYPEID_UNIT || !((Creature*)target)->isVehicle())
        return;
    VehicleKit* vehicle = (VehicleKit*)target;
