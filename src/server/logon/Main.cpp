...
char serviceLongName[] = "DiamondCore Logon service";
    ...
    #ifdef WIN32
    {
        ...
        if(Aff > 0)
        {
            ...
            if(GetProcessAffinityMask(hProcess,&appAff,&sysAff))
            {
                ...
                if(!curAff )
                {
                    sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not accessible for LogonServer. Accessible processors bitmask (hex): %x",Aff,appAff);
                }
                else
                {
