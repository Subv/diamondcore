...
//   doesn't make it looks well, so workarounds, Darion charges, looks better
    ...
    void NPCChangeTarget(uint64 ui_GUID)
    {
        ...
        if (Creature* pTemp = ((Creature*)Unit::GetUnit((*m_creature), ui_GUID)))
            if (pTemp->isAlive())
                if (SpellEntry const* pTarger = SelectSpell(m_creature, -1, -1, SELECT_TARGET_DONTCARE, 0, 0, 0, 0, SELECT_EFFECT_DONTCARE))
                    if (pvictim->isAlive())
                    {
						
                        ((Creature*)pTemp)->AddThreat(pvictim, 0.0f);
                        ((Creature*)pTemp)->AI()->AttackStart(pvictim);
                        ((Creature*)pTemp)->SetInCombatWith(pvictim);
                    }
    }
